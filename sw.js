...
self.addEventListener('fetch', (event) => {
 ...
 // Handle same-origin HTML with network-first strategy
...
async function handleImageRequest(event) {
 ...
 if (cachedResponse) {
   ...
   ongoingRequests.set(requestKey, updatePromise);
 ...
 const fetchPromise = fetch(event.request)
   ...
   .catch(async () => {
     ...
     throw new Error('Network error and no offline fallback');
   })
   .finally(() => {
     ongoingRequests.delete(requestKey);
   });
...
async function prefetchNearbyAudio(userLocation, locations) {
 if (!userLocation || !Array.isArray(locations)) return;
 
 // Find the closest 3 locations with audio
 const locationsWithDistance = locations
   .filter(loc => loc.audio)
   .map(loc => {
 ...
 const prefetchPromises = closestLocations.map(async (location) => {
   try {
